...
function [dis, v, qv, gamma_opt]=H2_Main_Geodesic(beta1,beta2,varargin)
...
if optRigid == 0 %no Rigid Motions
    [alpha2(:,:,1),q2]=...
        H2_gradientMethod(alpha1(:,:,1),q1,alpha2(:,:,1),q2,epsilon); % Gradient Method
else %optimizing over rigid motions
    alpha2(:,:,1) = alpha1(:,:,1); %align the starting points
    [q2] = H2_gradientMethod_ModG(q1,q2,epsilon);
end    
...
if doplot==1
    ...
    plot(x(1,1),y(1,1),'*k',x(1,S+1),y(1,S+1),'*k')
