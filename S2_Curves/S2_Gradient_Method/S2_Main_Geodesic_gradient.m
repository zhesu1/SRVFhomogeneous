...
function [dis,v,qv,gamma_opt]=S2_Main_Geodesic_gradient(beta1,beta2,varargin)
...
if optRigid == 0 %no Rigid Motions
    [alpha2(:,:,1),q2]=...
    S2_gradientMethod(alpha1(:,:,1),q1,alpha2(:,:,1),q2,epsilon); % gradient method
else %optimizing over rigid motions
    alpha2(:,:,1) = alpha1(:,:,1); %align the starting points
    [q2]=S2_gradientMethod_ModG(q1,q2,epsilon);
end    
...
if doplot==1
    ...
    plotS2
    plot3(betab(1,1,1),betab(2,1,1),betab(3,1,1),'*k',...
        betab(1,1,S+1),betab(2,1,S+1),betab(3,1,S+1),'*k');
